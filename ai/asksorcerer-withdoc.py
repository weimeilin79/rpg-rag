...
consumer = KafkaConsumer(
    'npc2-request',  # Kafka topic to consume from
...
loader = DirectoryLoader('./story', glob="./lyria*.md", show_progress=True)
...
chat_template = ChatPromptTemplate.from_messages(
    [
        SystemMessagePromptTemplate.from_template("You are a sorcerer in the following pieces of context, has been asked a question. You must provide an answer."
                "Your are female talks in pride tone. Don't answer if the question is not in the context."
...
def query_data(question):
    messages = chat_template.format_messages(context=docs, text=question)
